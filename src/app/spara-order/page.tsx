  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.success && data.data?.orderNumber) {
        router.push(`/spara-order/${data.data.orderNumber}`);
      } else {
        setError(data.error || `Kunde inte hitta någon beställning med ordernummer eller e-post "${query}". Kontrollera stavningen.`);
      }
    } catch {
      setIsSearching(false);
      setError("Nätverksfel vid sökning av order.");
    }
  };
