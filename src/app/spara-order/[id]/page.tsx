...
export default function OrderTrackingDetailPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch(`/api/tracking?id=${encodeURIComponent(params.id)}`);
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center space-y-4">
        ...
        <p className="text-xs font-mono text-ink/70">
          Vi kunde inte hitta någon beställning med nummer &ldquo;{params.id}&rdquo;.
        </p>
        <Link
          href="/spara-order"
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-stone pb-6">
          <div className="space-y-1">
            ...
            <p className="text-xs sm:text-sm text-ink/75 font-sans">
              Leveransdestination: <strong className="text-ink">{order.deliveryZoneName || order.customerCity || "Stockholm"}</strong>
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
          ...
          <div className="p-5 bg-stone-light/40 border border-stone space-y-3">
            ...
            <p className="text-[11px] font-mono text-ink/70">
              Leveranszon: <strong className="text-ink">{order.deliveryZoneName || order.customerCity || "Stockholm Innerstad"}</strong>
            </p>
          </div>
        </div>
      </div>
