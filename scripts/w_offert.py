...
write("src/app/tjanster/offert/page.tsx", """\"use client\";
...
export default function OffertPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const quoteNumber = `OFF-${new Date().getFullYear()}-${Math.floor(1000 + Math.random() * 9000)}`;

    const newQuote: QuoteRequest = {
      id: `quot-${quoteNumber.toLowerCase()}`,
      quoteNumber,
      isB2B,
