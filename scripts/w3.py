...
write("src/app/admin/produkter/page.tsx", """\"use client\";
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (isCreatingNew) {
      const newProd: Product = {
        ...
        model: model || name,
        category,
        categoryNameSwedish: category === "Fatolj" ? "Fåtöljer" : category === "Soffa" ? "Soffor" : category === "Tillbehor" ? "Dynsatser" : "Möbler",
        ...
        variants: [
          {
            ...
            materialDescription: "Klassiskt utförande",
            priceDelta: 0,
            sku: "SKD-NEW-01",
            inStock: true,
          }
      ...
      {(isCreatingNew || editingProduct) && (
        <form onSubmit={handleSave} className="bg-canvas border-2 border-stone p-6 sm:p-8 space-y-6 shadow-md">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="sm:col-span-2">
              <label className="block text-xs font-mono uppercase text-wood mb-1">Bild-URL *</label>
              <input
                type="url"
                required
                value={primaryImage}
